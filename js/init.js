...
jQuery(function() {
    ...
    {
	if (_settings.header.fullScreen)
	{
	    var $header = $('#header');
    ...
    $('#nav > ul').dropotron(_settings.dropotron);

    if (_isMobile)
    {
	$header.css('background-attachment', 'scroll');
	_settings.header.fullScreen = false;
    }
    
    // Homepage header
    if ($body.hasClass('homepage'))
    {
	if (_settings.header.fadeIn)
	{
	    overlay = $('<div class="overlay" />').appendTo($header);
